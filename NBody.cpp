...
// Parrallzie  the force calcuation
// Giving a thread for each F calulcation
...
int main(int argc, char **argv)
{
  ...
  for (int t = 0; t < tmax; t++)
  {
    ...
    { // force on body x due to
      ...
      { // all other bodies
        ...
        if (i != x)
        {
          ...
          x_diff = (body[i][Y_POS]) - (body[x][Y_POS]);
          x_diff = (body[i][Z_POS]) - (body[x][Z_POS]);
    ...
    FILE *fp2 = fopen("c_exec_time.csv", "a");
    if (fp2 == NULL)
    {
